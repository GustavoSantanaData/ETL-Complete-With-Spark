...
def calc_max_partition_value_mongodb(
    ...
    Returns:
        :return: max_estimated_to_injestion int: o valor máximo da coluna de partição
    """

    mensage = (
        f">>> [INFO] Calculating maximum partition column value {partition_column}"
    )
    update_logs(mensage, filename, storage_bucket_name, remote_log_file)
    ...
    try:
        ...
        if isinstance(max_result, str):
            try:
                ...
                mensage = (
                    f">>> [WARN] max_result ({max_result}) cant be converted to int."
                )
                update_logs(mensage, filename, storage_bucket_name, remote_log_file)
